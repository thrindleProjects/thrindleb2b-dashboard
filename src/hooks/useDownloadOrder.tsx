  ...
  const downloadPDF = useCallback(async () => {
    try {
      ...
      const result = await axios.post(
        '/api/getOrderReciept',
        {
          companyName: order.data.company.companyName || 'Customer',
          orderRefCode: order.data.orderRefCode,
          dateCreated: formatDateWithYear(order.data.createdAt || ''),
          listItems: order.data.listItems,
          listTotalPrice: listTotalPrice.toLocaleString('en-us'),
          deliveryFee: (order.data.deliveryFee || 0).toLocaleString('en-us'),
          serviceCharge: (order.data.serviceCharge || 0).toLocaleString(
            'en-us'
          ),
